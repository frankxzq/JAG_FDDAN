...
def load_data(image_file, label_file):
    ...
    '''
    ### PCA
    shapeor = data_all.shape
    data_all = data_all.reshape(-1, data_all.shape[-1])
    data_all = PCA(n_components=30).fit_transform(data_all)
    shapeor = np.array(shapeor)
    shapeor[-1] = 30
    data_all = data_all.reshape(shapeor)
    '''
